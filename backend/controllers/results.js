...
function calculateTotalRaceTime(attributes, trackData, weatherScore) {
  ...
  for (let lap = 0; lap < 10; lap++) {
    const lapTime = calculateLapTime(attributes, trackData, weatherScore);
...
function calculateLapTime(attributes, trackData, weatherScore) {
  ...
  const speedImpact = (50 - attributes.speed) * (0.05 + trackData.fullThrottle * 0.1); // Speed has higher impact with higher full throttle
  const maniabilityImpact = (50 - attributes.maniability) * trackData.technicalFactor * 0.03;
  const reliabilityImpact = (50 - attributes.reliability) * 0.02;
  const aerodynamicsImpact = (50 - attributes.aerodynamics) * 0.01;
  const driverSkillsImpact = (50 - attributes.driver_skills) * 0.04;
  const gearChangesImpact = (trackData.gearChangesPerLap - 47) * 0.01; // Adjust coefficient as needed
  const weatherImpact = (99 - weatherScore) * 0.1;
  ...
  lapTime += (Math.random() - 0.5) * 0.25 * 1000; // +/- 250 ms variability
